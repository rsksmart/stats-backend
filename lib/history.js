		...
		if( historyBlock )
		{
			...
			else
			{
				...
				else
				{
					...
					block.received = historyBlock.propagTimes[propIndex].received;
					this.setAvgNetworkHashrate(block.netHashRate);
					block.propagation = historyBlock.propagTimes[propIndex].propagation;
...
History.prototype.setAvgNetworkHashrate = function(hashrate)
{
	if (hashrate !== undefined && hashrate)
		this._networkHashrate = hashrate;
}

History.prototype.getAvgNetworkHashrate = function()
{
...
History.prototype.getBtcAvgNetworkHashrate = function()
{
	return this._btcHashrate;
}
...
History.prototype.getCharts = function()
{
	if(this._callback !== null)
	{
		...
		this._callback(null, {
			...
			avgHashrate : this.getAvgNetworkHashrate(),
			btcAvgHashrate : this.getBtcAvgNetworkHashrate(),
			hashrateComparedToBtcNetwork: this.getHashrateComparedToBtcNetwork()
		});
	}
}
...
History.prototype.updateBtcHashrate = function() {
	...
	https.get(source, (resp) => {
		...
		resp.on('end', () => {
			...
			if (!parsedData || !parsedData.hasOwnProperty("current_hashrate_estimated")) {
				return;
			}
			
			const oldBtcHashrate = this._btcHashrate;
			const btcHashrateInEH = parsedData.current_hashrate_estimated;
