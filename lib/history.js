		...
		if( historyBlock )
		{
			...
			else
			{
				...
				else
				{
					...
					block.received = historyBlock.propagTimes[propIndex].received;
					this.netHashRate = block.netHashRate;
					block.propagation = historyBlock.propagTimes[propIndex].propagation;
...
{
	const rskHashrate = this.getAvgHashrate();

	return rskHashrate * 100 / (this._btcHashrate === 0 ? 1 : this._btcHashrate);
}
...
History.prototype.updateBtcHashrateFromExternalSource = function(source)
{
	https.get(source, (resp) => {
		...
		resp.on('end', () => {
			...
			const btcHashrateGiga = JSON.parse(data).hash_rate;
